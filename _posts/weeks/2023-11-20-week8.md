...
2. Lets figure out how to debug this with jdb! Run: 
```
$ javac -g -cp .:lib/hamcrest-core-1.3.jar:lib/junit-4.13.2.jar *.java
$ jdb -classpath .:lib/hamcrest-core-1.3.jar:lib/junit-4.13.2.jar SemanticAnalysisHandlerTests
```

3. Right now you are in a jdb initialized enviornment, one thing you can do is place a breakpoint! A breakpoint is a point in the code where the program will pause before it executes that line!

Lets place a breakpoint at ChatHandler on line 70

```
> stop at ChatHandler:70
...
> run
```

Now you should be able to seeing something like this: 
```
run org.junit.runner.JUnitCore SemanticAnalysisHandlerTests
Set uncaught java.lang.Throwable
Set deferred uncaught java.lang.Throwable
>
VM Started: JUnit version 4.13.2
.Set deferred breakpoint ChatHandler:70
Breakpoint hit: "thread=main", ChatHandler.handleRequest(), line=70 bci=462
70            String[] chatHistoryArr = this.chatHistory.split("\n\n");
```

2. Use `jdb` with `SemanticAnalysisHandlerTests` to run _just that `[file_name]` file (you don't need to specify the folder `chathistory`, we already handled it)_ from the command line using the `main` method of `ChatServer`. Take use `jdb` commands to get the following information and take screenshots of:
