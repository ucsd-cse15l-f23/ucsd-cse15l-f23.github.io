...
2. Use `jdb` with `ChatServer` to run _just that `[file_name]` file(you don't need to specify the folder `chathistory`, we already handled it)_ from the command line using the `main` method of `ChatServer`. Take use `jdb` commands to get the following information and take screenshots of:
